...
const BookingForm = ({ currentUser, paymentIntent }: Props) => {
  ...
  const { mutate: bookRoom, isLoading } = useMutation(apiClinet.createRoomBooking, {
    onSuccess: () => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-5 rounded-lg border border-slate-300 p-5">
      ...
      <div className="space-y-2">
        ...
        <div className="bg-blue-200 p-4 rounded-md">
          <p className="font-semibold text-lg">Total Cost: ${paymentIntent.totalCost.toFixed(2)}</p>
